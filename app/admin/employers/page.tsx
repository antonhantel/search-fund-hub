...
export default async function EmployersPage() {
  ...
  return (
    <div className="px-4 py-6">
      ...
      {pending.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Pending Approval ({pending.length})</h3>
                ...
                <li key={employer.id} className="px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h4 className="text-lg font-medium text-gray-900">{employer.firstName && employer.lastName ? `${employer.firstName} ${employer.lastName}` : employer.companyName}</h4>
                      ...
                      {employer.industry && <p className="text-sm text-gray-600 mt-1">Industry: {employer.industry}</p>}
                      {employer.website && (
                        <a href={employer.website} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:text-blue-800 mt-1 inline-block">{employer.website}</a>
                      )}
                      {employer.description && <p className="text-sm text-gray-700 mt-2">{employer.description}</p>}
                    </div>
                    <div className="flex gap-2 ml-4">
                      <Link href={`/admin/employers/${employer.id}`} className="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Edit</Link>
      ...
      {approved.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Approved ({approved.length})</h3>
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            <ul className="divide-y divide-gray-200">
      ...
      {rejected.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Rejected ({rejected.length})</h3>
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            <ul className="divide-y divide-gray-200">
